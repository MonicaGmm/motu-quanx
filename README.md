...
**穷举规模（累计约 1900 万组，全部落空）**：

| 轮次 | 做法 | 组合数 | 命中 |
|---|---|---|---|
| 1 | 真实响应向量 × 数十种密钥候选 × 上千种拼接 | 4 万+ | 0 |
| 2 | 11 个候选字段的有序子集(1~4) × 8 分隔符 × 原串/`k=v&` × 4 种无密钥哈希 ∪ HMAC×25 类密钥 | **10,888,704** | 0 |
| 3 | 从 109.5MB 主二进制抽 **UTF-8 cstring + UTF-16LE(Swift ustring)**，筛出 78,018 个高熵串取 Top 15000 当密钥 × 517 种消息构造 | **7,755,000** | 0 |

**顺带得到的硬结论**（换会话密钥做差分，拿真服务端当判官）：

| 改什么 | 服务端 | 说明 |
|---|---|---|
| 只换 `encryptedKey`（重新包装同一个会话密钥） | **200** ✅ | 签名**不覆盖** encryptedKey —— 这是透明转发能成立的原因 |
| 换一个全新会话密钥（明文参数 `{}` 一字未动） | **400 `sign invalid`** | 签名**覆盖密文** —— 连请求参数都改不了 |
